#!/usr/bin/env bash
# AArch64 build for Freedom Planet 2 (Unity 6 / Mali-450).
#
...
OUTPUT=${FP2_OUTPUT:-build/fp2-nextos}
export LC_ALL=C
export TZ=UTC
export SOURCE_DATE_EPOCH=${SOURCE_DATE_EPOCH:-1786233600}

fail() {
  printf 'fp2 build error: %s\n' "$*" >&2
  exit 1
}

case $OUTPUT in
  /*|*../*|../*|*/..|..)
    fail "FP2_OUTPUT must stay inside the port directory"
    ;;
esac
...
NXSR=${NEXTOS_SYSROOT:-}
if [[ -z $NXSR ]]; then
  # Escolher pelo sysroot que REALMENTE tem os headers: o mais novo por sort -V
  # nem sempre traz SDL2/EGL/GLES, e ai o build falha por escolha, nao por falta.
...
mapfile -t SOURCES < <(
  find src -maxdepth 1 -type f -name '*.c' \
    ! -name 'diagnostics.c' ! -name 'probe.c' -print | sort
)
...
for source in "${SOURCES[@]}"; do
  ...
  "$CC" -std=gnu11 --sysroot="$NXSR" \
    ...
    -DFP2_RELEASE_BUILD=1 -ffunction-sections -fdata-sections \
...
"$CC" --sysroot="$NXSR" -fPIE -pie -o "$OUTPUT" "${OBJS[@]}" \
